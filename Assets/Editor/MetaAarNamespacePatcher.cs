...
/// <summary>
/// Patches the duplicate <c>com.oculus.Integration</c> Android namespace that the Meta XR
/// SDK ships in two prebuilt AARs. Both <c>OVRPlugin.aar</c> (Meta XR Core SDK) and
/// <c>InteractionSdk.aar</c> (Meta XR Interaction SDK) declare the same <c>package=</c>
/// attribute. Since Unity 6.4 (6000.4.4f1+) bundles Gradle 9 / AGP 9, this duplicate
/// namespace is a hard build error ("Namespace 'com.oculus.Integration' is used in multiple
/// modules and/or libraries") instead of a warning, so Android export fails.
///
/// This runs before every Android build: it unpacks each AAR, rewrites the conflicting
/// package attribute to a unique value, and repacks it. Idempotent and self-reapplying
/// (Library/PackageCache AARs are restored on cache wipe; this patches them again next build).
///
/// Workaround source: Unity staff on the official thread, mirrored on the Meta
/// oculus-samples/Unity-FirstHand#17 issue. REMOVE once Meta ships unique namespaces in the SDK.
/// </summary>
        ...
        try
        {
            ...
            if (!changed)
            {
                UnityEngine.Debug.Log(
                    $"{Tag} {Path.GetFileName(aarPath)} already has namespace {newNamespace}, skipping.");
                return;
            }
