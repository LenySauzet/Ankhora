...
namespace Ankhora.Foundation.Recording
{
    ...
    public class RecordingSession
    {
        ...
        private bool _voiceCaptureStarted;
        ...
        public void Begin(float now)
        {
            ...
            _rightSkeleton = null;
            _recorder.Begin(now);
            _voiceCaptureStarted = false;
            if (_voice != null && _voice.IsAvailable)
            {
                // A voice-source failure must never sink the hands take — the core promise is hands-only fallback.
                try { _voice.BeginCapture(now); _voiceCaptureStarted = true; }
                catch (Exception e) { Debug.LogWarning($"[RecordingSession] Voice capture start failed, recording hands-only: {e.Message}"); }
            }
        }
        ...
        public bool SaveTo(MasterclassStore store, float now, out int frameCount, out string error)
        {
            ...
            // Only finalize voice if capture actually started, and never let a voice failure abort the hands save.
            try
            {
                if (_voiceCaptureStarted && _voice.TryEndCapture(now, out VoiceCaptureResult voiceResult) &&
            ...
            {
                Debug.LogWarning($"[RecordingSession] Voice finalize failed, saving hands-only: {e.Message}");
            }
